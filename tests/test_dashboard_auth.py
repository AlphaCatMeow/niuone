...
"""
    ...
    def test_vue_router_and_components_own_independent_category_routes(self):
        ...
        self.assertIn('role="dialog"', mainline_page)
        self.assertNotIn('aria-label="关闭未覆盖原因"', mainline_page)
        ...
        self.assertIn("ranking.key === 'today' ? '今日领涨列表' : '结构代表股'", mainline_page)
        ...
        self.assertIn('.theme-stock-details { position:absolute; z-index:10; top:calc(100% + 3px); right:0; left:0; width:auto;', mainline_page)
        ...
        self.assertIn('grid-template-areas:"identity score" "metrics metrics" "stocks stocks" "context context"', mainline_page)
        ...
        self.assertIn('.theme-ranking-panel { min-width:0; overflow:visible; border:1px solid var(--mainline-line);', mainline_page)
        ...
        self.assertIn('.theme-row { display:grid; grid-template-columns:minmax(0,1fr) auto;', mainline_page)
        ...
        self.assertIn('.theme-stock-detail-row { padding:7px 8px; border-radius:6px; background:var(--panel2);', mainline_page)
