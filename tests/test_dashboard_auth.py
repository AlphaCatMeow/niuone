...
"""
    ...
    def test_vue_router_and_components_own_independent_category_routes(self):
        ...
        self.assertIn("activeFilter === 'today' ? '等效上涨' : '归因强股'", mainline_page)
        self.assertIn("activeFilter === 'today' ? '日内领涨' : '结构龙头'", mainline_page)
        self.assertIn(':aria-expanded="expandedTheme === theme.industry"', mainline_page)
        self.assertIn("activeFilter === 'today' ? '今日领涨列表' : '结构代表股'", mainline_page)
        ...
        self.assertIn('data-tooltip="结构广度按个股题材归因权重计算；今日广度还会按题材样本量', mainline_page)
