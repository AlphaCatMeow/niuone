...
9. 外部确认只做限幅覆盖：读取已完成的隔夜美股盘面及明确A股行业映射，并对首轮前5候选读取近3日个股消息面。正向外盘/消息不能把落后行业、追高或硬过滤候选变成买点；隔夜防守、负行业映射和明确利空必须降权并写入风险。""" if sector_tide_enabled else ""
    ...
    niuone_strategy_section = """牛牛战法（强势股→强势行业→龙头梯队→个股买点）：
1. 主线不是涨幅榜第一名：必须由多只强势股在20/5日相对强度、成交参与、趋势和新高上共同确认；单只股票独强会触发集中度惩罚，不能确认主线。
2. 日内强势只记为intraday_mainline观察，不直接升级主线；只有相邻交易日继续强势且至少2只核心强股延续，才确认mainline。没有跨日确认的mainline不得用牛牛领航强行开仓。
3. 牛牛领航：只做进攻/轮动行情的跨交易日已确认主线，个股必须处于该行业strong_score前三且仍为强势股；第一名优先，第一名涨停或无有效买点时可顺延，只买有效突破或首次EMA20缩量回踩；单日涨幅>4%或距EMA20>1ATR不追，30%是单票绝对上限。
4. 牛牛回踩：主线仍为mainline/diverging且分数≥70，只参与行业前三龙头梯队的EMA20承接或重新收复；单日涨幅>4%或距EMA20>1ATR不追；25%是单票绝对上限。
5. 牛牛启动：主题必须处emerging并已跨交易日延续，至少两只强势股保持共振，只允许行业前三龙头梯队突破/收复时建立观察仓；单日涨幅>7%或距EMA20>1.5ATR不追；15%是单票绝对上限，升级主线后才允许加仓。
...
9. 退出服从龙头梯队与主线状态：连续两个交易日跌出行业前三龙头梯队时换出；主题连续转弱、市场硬停止+主题转弱、时间窗未兑现、结构止损、2R减半与2ATR跟踪均由本地执行层复核。""" if niuone_enabled else ""
    if strategy_suite == STRATEGY_SOURCE_PRESET_TEXT:
