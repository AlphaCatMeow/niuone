...
`/niuone-mainline` is a full-market research read model independent of both the active trading strategy and the final BUY universe. During A-share trading sessions, the Dashboard obtains one coverage-validated Tencent per-stock quote batch every 30 seconds by default and uses it for both Market Sentiment and the fast Theme Strength calculation. The fast path reads only local daily K-lines, industry mappings, and slow confirmation factors from the latest complete scan; it makes no additional market-data or model call. The page selects the newest valid minute or complete-research snapshot and separately presents the Mainline Brewing → Mainline Markup → Mainline Climax → Mainline Divergence → Mainline Fade lifecycle, cross-session confirmed mainlines, intraday strength, intraday V-recovery observations, representative stocks, and coverage; that observation is not a NiuOne Probe entry condition. Insufficient coverage, stale quotes, or a calculation failure retains the previous valid theme result. Raw per-stock quotes, private SQLite data, and trade-candidate status never reach the browser. `DASHBOARD_MARKET_BREADTH_SAMPLE_INTERVAL_SECONDS` configures the shared interval from 30 through 600 seconds; setting `DASHBOARD_NIUONE_MAINLINE_MINUTE_REFRESH_ENABLED=0` disables the fast theme refresh after a restart.

Each daily dragon-tiger snapshot identifies stocks repeated on adjacent A-share trading days, and Vue retains a separate consecutive-list badge for that information. A news precheck is triggered for either a limit-up-streak stock (`limit_up_streak >= 2`) or a consecutive-listing stock (`consecutive_listed = true` and `consecutive_list_days >= 2`). The list shows a **✦ Positive** or **▼ Negative** badge and matching left-edge highlight only when a structured result is checked, available, and has `positive` or `negative` tone. Hovering or keyboard-focusing that directional badge shows the latest three-day summary; limit-up and consecutive-list badges never open news. Expanding an eligible stock still exposes its complete precheck state and details, including neutral, pending, and unavailable results. The view separates core news, direct impact, and market sentiment: company and exchange disclosures plus mainstream financial media form the factual layer, while Xueqiu and X/Twitter remain a sentiment-only layer. The batch start time comes from the dragon-tiger schedule configured by `IWENCAI_DRAGON_TIGER_CRON`, never from the upstream `generated_at`. After a successful pull, model calls cover only unchecked stocks matching either condition. Scheduler startup and the next dragon-tiger pull both inspect the persisted pending state, so missed Friday checks can complete after a weekend restart or before the next trading day. Once complete, no same-day model calls occur. Browser polling only reads the result and never triggers a query.
...
The indices page adds a dedicated **Market Sentiment** switch immediately after **Fund Flow**. This view presents real A-share samples every 30 seconds by default: the upper dual-axis panel displays limit-ups, limit-downs, broken limit-ups, red stocks, and green stocks. The lower shared-time-axis turnover panel overlays projected full-day turnover, today's actual cumulative turnover, a dashed previous-day same-progress turnover curve, projected increment, and the same-time difference of today's actual turnover minus the prior curve; positive means expanding volume and negative means contracting volume. The history file retains up to 600 current-day aggregate points plus one compact prior-trading-day actual-turnover curve and does not copy prior breadth or limit-state data. The full-day comparison reference is independent of the projection model's training samples and remains the same throughout a trading day. Actual turnover primarily uses Eastmoney one-minute Shanghai/Shenzhen index turnover with Tencent full-market turnover as fallback. From 09:30 through 09:34, today's only live input is the finalized 09:25 full-market auction amount; the service applies 0.5-elasticity square-root shrinkage against the median auction and full-day turnover from up to the latest 20 valid matched trading days. From 09:35 onward it switches to the latest 20 complete trading days' five-minute cumulative turnover distribution model. The API exposes the active stage's actual source, projection model, sample range, sample count, and interval, and the turnover panel displays that methodology; estimates from different projection models are not joined into one line. The FastAPI-native endpoint is `/api/market_breadth`, and the deep link is `/indices?panel=market-breadth`.

After the current day's 20-day turnover-distribution profile is built successfully for the first time, it is atomically saved under the private runtime directory. A restarted Dashboard restores only a validated profile for the same trading day and recomputes the projection from the latest actual turnover; failed upstream requests never clear or overwrite that valid cache.
