...
Signal replay causally precomputes technical indicators once for each stock's complete historical series and exposes only the read-only prefix visible on each date. This avoids recomputing BBI, EMA, and KDJ from the first bar on every session. Built-in stateless scorers skip unneeded pre-signal scoring, while NiuOne and Sector Tide still replay the full warmup range to preserve cross-session market and mainline state.
