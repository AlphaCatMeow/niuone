...
指数行情页在“资金流动”右侧提供独立的“市场情绪”切换按钮。该视图展示 60 秒真实采样的 A 股市场情绪曲线：上方双纵轴呈现涨停、跌停、炸板、红盘和绿盘数量；下方共享时间轴的量能区间同时呈现预测全天量能、今日实际累计量能、虚线表示的前日同期量能、预测增量，以及“较昨日同期差”（今日实际量能减去前一交易日同进度量能，正值放量、负值缩量）。历史文件仅额外保留最近一个交易日的精简实际量能曲线，不复制前日红绿盘等市场宽度数据；比较基准独立于预测模型训练样本，同一交易日保持一致。实际量能优先取东方财富沪深指数 1 分钟成交额，腾讯全市场成交额兜底；09:30～09:34 的预测只使用已经确定的 09:25 全市场竞价成交额作为今日实时输入，并以最近最多 20 个有效配对交易日的竞价额和全天成交额中位数执行 0.5 弹性的平方根收缩，09:35 起切换为最近 20 个完整交易日的 5 分钟累计成交分布模型。接口同时返回当前阶段的实际来源、预测模型、样本起止日期、样本数和时间间隔，页面在量能区下方展示这些口径；不同预测模型的旧数据不会被拼接为同一条曲线。对应的 FastAPI 接口为 `/api/market_breadth`，深链接使用 `/indices?panel=market-breadth`。
