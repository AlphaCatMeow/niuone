...
`/niuone-mainline` 是独立于当前交易策略和最终买入范围的全市场研究读模型。Dashboard 默认在 A 股交易时段每 30 秒取得一份通过覆盖校验的腾讯逐股行情，同时驱动市场情绪和题材强度快速计算；快速路径只读取本地日 K、行业映射和最近完整扫描的慢速确认因子，不额外调用行情或模型。页面从分钟快照与完整研究快照中选择生成时间最新的有效结果，分别展示“主线酝酿 → 主线主升 → 主线高潮 → 主线分歧 → 主线退幕”生命周期、跨日确认主线、日内强度、日内 V 形修复观察、代表股和覆盖率；该观察不作为牛牛试仓的交易条件。覆盖不足、行情过期或计算失败时继续使用上一份有效题材结果；浏览器不会拿到逐股原始行情、私有 SQLite 或交易候选状态。共享间隔可通过 `DASHBOARD_MARKET_BREADTH_SAMPLE_INTERVAL_SECONDS` 配置为 30～600 秒，`DASHBOARD_NIUONE_MAINLINE_MINUTE_REFRESH_ENABLED=0` 可在重启后关闭题材快速刷新。
