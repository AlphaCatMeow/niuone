...
|---|---|---:|---:|---:|---|
| `capital-flow.gif` | `/indices` | 1200 × 750 | 8 fps | 10–13 秒 | 切换 A 股、查看板块与活跃股票、切换资金流动、重播行业动画 |
...
3. 停留约 1–2 秒，同时展示板块涨跌幅与成交额、换手率、成交量活跃股票榜。
4. 点击“资金流动”，完整展示左侧净流出行业与右侧净流入行业。
5. 点击“重播”，连续截取进度线和柱状图变化。
...
3. 确认最新统计包含跌停板、涨停板、炸板、红盘、绿盘、预测全天量能、今日实际量能、前日同期量能、预测增量和较昨日同期差。
