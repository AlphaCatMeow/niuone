...
实战 B1 选股计划运行在 Dashboard 进程内；每个计划时间会在买卖决策前同步生成统一的“此刻盘面总结与评价”，其风险标签直接作为实战交易上下文。页面按钮和手动选股与交易链路也使用同一生成器。定时调度器不负责选股，但负责独立的模拟持仓自动离场检查。要让模拟账户完整走通“定时选股—盘面总结评价—决策—自动离场”，Dashboard 与定时调度器都必须持续运行。
