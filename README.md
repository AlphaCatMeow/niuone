...
以下演示均为暗色主题的独立动图，按 1200 像素宽输出；根据功能使用完整页面或聚焦面板视野，并保留鼠标移动、点击反馈和页面滚动过程。点击动图可进入对应的在线页面。
...
<p align="center"><sub>在账户总览、收益曲线与当前持仓同屏展示的基础上，点击切换每日 / 累计收益并打开交易日历。</sub></p>
...
<p align="center"><sub>并排对照今日强度与跨日结构前五，结合东财即时排名、上涨广度和领涨股交叉校验，展开两类代表股并查看未覆盖原因。</sub></p>
...
<p align="center"><sub>鼠标沿日内曲线移动，实时查看对应时间点的涨跌停、炸板、红绿盘家数与预测 / 实际 / 增量量能。</sub></p>
...
<p align="center"><sub>点击展开盘后总结，再滚动查看核心判断、资金流向、热门板块、风险提醒与次日关注项。</sub></p>
...
<p align="center"><sub>按时间流查看推文、回复和引用内容，展开完整正文，并在图文推文中打开图片预览。</sub></p>
...
<p align="center"><sub>对照当前价、目标价与目标空间，点击查看机构、分析师、看多逻辑和风险点，并切换历史日报。</sub></p>
...
<p align="center"><sub>点击进入策略配置并切换策略，返回总览浏览全部分组，再打开行情与资金流设置。</sub></p>

> 动图中的行情与模拟账户数据仅用于界面展示，不构成投资建议；配置中心动图来自隔离的临时运行目录，不含密钥或私有运行数据。

## 功能概览

- **统一看板**：集中展示题材强度、指数、板块、市场热度、行业资金流、龙虎榜和历史消息。
- **题材强度与策略研究**：默认复用 30 秒全市场行情采样更新“今日排名”，并与用于跨日确认的“结构排名”并排展示；个股按题材归因权重聚合，东方财富概念涨幅榜只作即时交叉校验，不改变题材排序或交易门槛。内置基础、Z 哥、李大霄、板块潮汐和牛牛战法，牛牛战法包含日线区间 V 型试仓、跨日主线确认以及启动/领涨/转强。
